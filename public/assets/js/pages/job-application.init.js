...
var isChoiceEl = document.getElementById('idStatus'),
  ...
  perPage = ((choices = new Choices(isTypeEl, { searchEnabled: !1 })), 8),
  editlist = !1,
  options = {
    valueNames: [
      'id',
...
function filterData() {
  var e = document.getElementById('idStatus').value,
    t = document.getElementById('idType').value,
    a = document.getElementById('demo-datepicker').value,
    n = a.split(' to ')[0],
    i = a.split(' to ')[1];
  (applicationList.filter(function (l) {
    matchData = new DOMParser().parseFromString(l.values().status, 'text/html');
    var s = matchData.body.firstElementChild.innerHTML,
      o = !1,
      c = !1,
      d = !1;
    return (
      (o = 'all' == s || 'all' == e || s == e),
      (c = 'all' == l.values().type || 'all' == t || l.values().type == t),
      (d =
        new Date(l.values().date.slice(0, 12)) >= new Date(n) &&
        new Date(l.values().date.slice(0, 12)) <= new Date(i)),
      o && c && d
        ? o && c && d
        : o && c && '' == a
          ? o && c
...
function ischeckboxcheck() {
  Array.from(document.getElementsByName('checkAll')).forEach(function (e) {
    e.addEventListener('change', function (t) {
      ...
      var a = document.querySelectorAll('[name="checkAll"]:checked').length;
...
function refreshCallbacks() {
  (Array.from(removeBtns).forEach(function (e) {
    e.addEventListener('click', function (e) {
      ...
      var t = applicationList.get({ id: itemId });
      ...
      e.addEventListener('click', function (e) {
        ...
        Array.from(t).forEach(function (e) {
          ...
          if (t == itemId) {
            ...
            var n = val.body.firstElementChild.innerHTML;
            (statusVal.setChoiceByValue(n),
              example && example.destroy(),
              (example = new Choices(typeField, { searchEnabled: !1 })));
            var i = e._values.type;
...
function deleteMultiple() {
  ids_array = [];
  var e = document.querySelectorAll('.form-check [value=option1]');
      ...
      if (e.checkValidity())
        if (
          ...
          if (
            ...
            var a = applicationList.get({ id: idField.value });
          ...
          (applicationList.add({
            id:
              '<a href="#" class="fw-medium link-primary">#VZ' + count + '</a>',
            company:
              '<div class="d-flex align-items-center">                    <div class="flex-shrink-0">                        <img src="' +
              companyLogoImg.src +
              '" alt="" class="avatar-xxs rounded-circle image_src object-fit-cover">                    </div>                    <div class="flex-grow-1 ms-2 name">' +
