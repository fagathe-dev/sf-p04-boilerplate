...
function loadCharts() {
  ...
  if (e) {
    ...
    new ApexCharts(document.querySelector('#chart-seller1'), t).render();
  }
  var r = getChartColorsArray('chart-seller2');
  if (r) {
    var a = {
    ...
    new ApexCharts(document.querySelector('#chart-seller2'), a).render();
  }
  var n = getChartColorsArray('chart-seller3');
  if (n) {
    var s = {
    ...
    new ApexCharts(document.querySelector('#chart-seller3'), s).render();
  }
  var l = getChartColorsArray('chart-seller4');
  if (l) {
    var o = {
    ...
    new ApexCharts(document.querySelector('#chart-seller4'), o).render();
  }
  var i = getChartColorsArray('chart-seller5');
  if (i) {
    var d = {
    ...
    new ApexCharts(document.querySelector('#chart-seller5'), d).render();
  }
  var c = getChartColorsArray('chart-seller6');
  if (c) {
    var u = {
    ...
    new ApexCharts(document.querySelector('#chart-seller6'), u).render();
  }
  var h = getChartColorsArray('chart-seller7');
  if (h) {
    var g = {
    ...
    new ApexCharts(document.querySelector('#chart-seller7'), g).render();
  }
  var m = getChartColorsArray('chart-seller8');
  if (m) {
    var p = {
    ...
    new ApexCharts(document.querySelector('#chart-seller8'), p).render();
  }
}
var url = '/assets/json/',
  sellerListData = '',
  ...
  for (
    ...
    if (e[a]) {
      ...
      document.getElementById('seller-list').innerHTML +=
        ...
        '" alt="' +
        e[a].shop[0].img_alt +
        '" height="45">                <h5 class="mb-1 mt-4"><a href="apps-ecommerce-seller-details.html" class="link-primary">' +
        e[a].shop[0].name +
        ...
        e[a].wallet_balance +
        '</h5>                        <span class="text-muted">Wallet Balance</span>                    </div>                </div>                <div class="mt-4">                    <a href="apps-ecommerce-seller-details.html" class="btn btn-light w-100">View Details</a>                </div>            </div>        </div>    </div>';
    }
  (paginationEvents(),
    pageEvent(e),
    selectedPage(),
...
function findNextId() {
  if (0 === sellerListData.length) return 0;
  var e = fetchIdFromObj(sellerListData[sellerListData.length - 1]),
...
function paginationEvents() {
  ...
  (prevButton.addEventListener('click', function () {
    currentPage > 1 &&
      (currentPage--, loadSellerList(sellerListData, currentPage));
  }),
...
function pageEvent(e) {
  ...
  var t = document.getElementById('page-num');
  t.innerHTML = '';
  for (var r = Math.ceil(e.length / itemsPerPage), a = 1; a < r + 1; a++)
    t.innerHTML +=
      "<div class='page-item'><a class='page-link clickPageNumber' href='javascript:void(0);'>" +
      a +
      '</a></div>';
...
searchResultList.addEventListener('keyup', function () {
  var e,
    t = searchResultList.value.toLowerCase();
  loadSellerList(
    ((e = t),
    sellerListData.filter(function (t) {
      return (
        -1 !== t.shop[0].name.toLowerCase().indexOf(e.toLowerCase()) ||
        -1 !== t.seller.toLowerCase().indexOf(e.toLowerCase())
      );
    })),
    currentPage,
  );
...
categoryInput.passedElement.element.addEventListener(
  ...
  function (e) {
    ...
    if ('All' != e.detail.value)
      var r = sellerListData.filter((e) => e.category == t);
    else r = sellerListData;
    loadSellerList(r, currentPage);
  },
  !1,
);
