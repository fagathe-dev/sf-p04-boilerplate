...
function tConvert(e) {
  ...
  var a = time_s.split(':'),
    ...
    i = a[1],
    o = n >= 12 ? 'PM' : 'AM';
  return (n = (n %= 12) || 12) + ':' + (i = i < 10 ? '0' + i : i) + ' ' + o;
}
...
if (
  ...
  (Invoices.push(invoice_new_obj), localStorage.removeItem('new_data_object'));
} else {
  if (
    ((Invoices = []),
    ...
    invoice_new_obj = JSON.parse(localStorage.getItem('new_data_object'));
...
(Array.from(Invoices).forEach(function (e) {
  ...
  if (e.img)
    ...
    var n = e.customer.split(' '),
      i = null;
    if (n.length >= 2) i = n[0].slice(0, 1) + n[1].slice(0, 1);
    else i = n[0].slice(0, 1);
    a =
  ...
  document.getElementById('invoice-list-data').innerHTML += o;
}),
...
function SearchData() {
  ...
  (invoiceList.filter(function (i) {
    ...
    var o = matchData.body.firstElementChild.innerHTML,
      r = !1,
      s = !1;
    return (
      (r = 'all' == o || 'all' == e || o == e),
...
function refreshCallbacks() {
  Array.from(removeBtns).forEach(function (e) {
    e.addEventListener('click', function (e) {
      ...
      var t = invoiceList.get({ id: itemId });
...
function ViewInvoice(e) {
  ...
  (localStorage.setItem('invoices-list', JSON.stringify(Invoices)),
    ...
    localStorage.setItem('invoice_no', t),
