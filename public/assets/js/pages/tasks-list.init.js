...
function filterOrder(e) {
  ...
  (tasksList.filter(function (e) {
    ...
    var a = matchData.body.firstElementChild.innerHTML;
    return 'All' == a || 'All' == t || a == t;
  }),
    tasksList.update());
}
function updateList() {
      ...
      if (e.checkValidity())
        if (
          ...
          if (
            ...
            var a = tasksList.get({ id: idField.value });
...
function SearchData() {
  var e = document.getElementById('idStatus').value,
    t = document.getElementById('demo-datepicker').value,
    a = t.split(' to ')[0],
    s = t.split(' to ')[1];
  (tasksList.filter(function (i) {
    matchData = new DOMParser().parseFromString(i.values().status, 'text/html');
    var l = matchData.body.firstElementChild.innerHTML,
      n = !1,
      r = !1;
    return (
      (n = 'all' == l || 'all' == e || l == e),
      (r =
        new Date(i.values().due_date.slice(0, 12)) >= new Date(a) &&
        new Date(i.values().due_date.slice(0, 12)) <= new Date(s)),
      n && r ? n && r : n && '' == t ? n : r && '' == t ? r : void 0
    );
  }),
    tasksList.update());
}
        ...
        e.addEventListener('click', function (e) {
          ...
          Array.from(t).forEach(function (e) {
            ...
            if (t == itemId) {
              ...
              var a = project.body.firstElementChild.innerHTML;
              (statusVal.setChoiceByValue(s),
                ...
                Array.from(
                  ...
                  var a =
                    t.parentElement.querySelector('.flex-grow-1').innerHTML;
                  new DOMParser()
              ...
              var s = val.body.firstElementChild.innerHTML;
              (statusVal.setChoiceByValue(s),
              ...
              var i = val.body.firstElementChild.innerHTML;
              (example.setChoiceByValue(i),
...
function fomateDate(e) {
  ...
  return t.getDate() + ' ' + a + ', ' + t.getFullYear();
}
function assignToUsers() {
  var e = [],
    t = document.querySelectorAll('input[name="assignedTo[]"]:checked');
  Array.from(t).forEach(function (t) {
    var a = t.parentElement,
      s = a.querySelector('.avatar-xxs').getAttribute('src'),
      i = a.querySelector('.flex-grow-1').innerHTML,
      l = {};
    ((l.assigneeName = i), (l.assigneeImg = s), e.push(l));
  });
  var a = e,
...
function deleteMultiple() {
  ...
  for (i = 0; i < e.length; i++)
    if (1 == e[i].checked) {
      var t =
        e[i].parentNode.parentNode.parentNode.querySelector('td a').innerHTML;
      ids_array.push(t);
    }
  'undefined' != typeof ids_array && ids_array.length > 0
    ? (Swal.fire({
        title: 'Are you sure?',
