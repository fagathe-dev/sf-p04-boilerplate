...
(searchProductList.addEventListener('keyup', function () {
  ...
  (productCategoryInput = new Choices(
    ...
    function (e) {
      ...
      if (e.detail.value)
        0 == (l = allproductlist.filter((e) => e.category === t)).length
          ? ((document.getElementById('noresult').style.display = 'block'),
            (document.getElementById('loadmore').style.display = 'none'))
          : ((document.getElementById('noresult').style.display = 'none'),
            (document.getElementById('loadmore').style.display = 'block'));
      else var l = allproductlist;
      loadProductData(l);
    },
    !1,
  ));
var productCategoryInput,
  productFileTypeInput = new Choices(document.getElementById('file-type'), {
    searchEnabled: !1,
  });
(productFileTypeInput.passedElement.element.addEventListener(
  ...
  function (e) {
    ...
    if (e.detail.value)
      0 ==
      (l = allproductlist.filter((e) => e.productImg.split('.').pop() == t))
        .length
        ? ((document.getElementById('noresult').style.display = 'block'),
          (document.getElementById('loadmore').style.display = 'none'))
        : ((document.getElementById('noresult').style.display = 'none'),
...
productSalesInputInput.passedElement.element.addEventListener(
  ...
  function (e) {
    var t = e.detail.value;
    if (e.detail.value)
      0 == (l = allproductlist.filter((e) => e.salesType === t)).length
        ? ((document.getElementById('noresult').style.display = 'block'),
          (document.getElementById('loadmore').style.display = 'none'))
        : ((document.getElementById('noresult').style.display = 'none'),
          (document.getElementById('loadmore').style.display = 'block'));
...
var minCostInput = document.getElementById('minCost'),
  ...
  filterDataAll = '';
function mergeTooltips(e, t, l) {
  var o = 'rtl' === getComputedStyle(e).direction,
    n = 'rtl' === e.noUiSlider.options.direction,
    a = 'vertical' === e.noUiSlider.options.orientation,
    r = e.noUiSlider.getTooltips(),
    d = e.noUiSlider.getOrigins();
  (Array.from(r).forEach(function (e, t) {
    e && d[t].appendChild(e);
  }),
    e &&
      e.noUiSlider.on('update', function (e, d, s, i, c) {
        var u = [[]],
          m = [[]],
          p = [[]],
          y = 0;
        r[0] && ((u[0][0] = 0), (m[0][0] = c[0]), (p[0][0] = e[0]));
        for (var g = 1; g < c.length; g++)
          ((!r[g] || c[g] - c[g - 1] > t) &&
            ((u[++y] = []), (p[y] = []), (m[y] = [])),
            r[g] && (u[y].push(g), p[y].push(e[g]), m[y].push(c[g])));
        Array.from(u).forEach(function (e, t) {
          for (var d = e.length, s = 0; s < d; s++) {
            var i = e[s];
            if (s === d - 1) {
              var c = 0;
              Array.from(m[t]).forEach(function (e) {
                c += 1e3 - e;
              });
              var u = a ? 'bottom' : 'right',
                y = n ? 0 : d - 1,
                g = 1e3 - m[t][y];
              ((c = (o && !a ? 100 : 0) + c / d - g),
                (r[i].innerHTML = p[t].join(l)),
...
function loadMoreBtn() {
  ...
  if (e) {
    ...
    e.addEventListener('click', function (e) {
      ...
      if (l) {
        for (var o = t; o < t + 5; o++) l[o] && (l[o].style.display = 'block');
        (t += 5) >= l.length && (event.target.style.display = 'none');
      }
    });
  }
}
