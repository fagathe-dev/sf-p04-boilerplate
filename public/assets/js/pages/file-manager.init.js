function getChartColorsArray(e) {
  if (null !== document.getElementById(e)) {
    ...
    return (t = JSON.parse(t)).map(function (e) {
      var t = e.replace(' ', '');
      if (-1 === t.indexOf(',')) {
        var l = getComputedStyle(document.documentElement).getPropertyValue(t);
        return l || t;
      }
      var i = e.split(',');
      if (2 == i.length) {
...
function loadFileData(e) {
  ((document.querySelector('#file-list').innerHTML = ''),
    Array.from(e).forEach(function (e, t) {
      ...
      } else l = '<i class="ri-folder-2-fill align-bottom text-warning"></i>';
  ...
  Array.from(document.querySelectorAll('.file-manager-menu a')).forEach(
    function (e) {
      e.addEventListener('click', function () {
        ...
        var l = e.querySelector('.file-list-link').innerHTML;
        if (
          ...
          var i = allFileList.filter((e) => e.filetype === l);
          document.getElementById('folder-list').style.display = 'none';
        } else if ('Important' == l) {
          i = allFileList.filter((e) => 1 == e.starred);
          document.getElementById('folder-list').style.display = 'none';
        } else {
          i = allFileList;
...
function editFolderList() {
  Array.from(document.querySelectorAll('.folder-card')).forEach(function (e) {
    Array.from(e.querySelectorAll('.edit-folder-list')).forEach(function (t) {
      t.addEventListener('click', function (t) {
        var l = e.querySelector('.card').getAttribute('id').split('-')[1];
        l ==
          e
          ...
          (document.getElementById('createFolderModalLabel').innerHTML =
            'Folder Rename'),
          (document.getElementById('folderid-input').value = 'folder-' + l),
          (document.getElementById('foldername-input').value =
            e.querySelector('.folder-name').innerHTML));
      });
    });
  });
}
(Array.prototype.slice.call(createFolderForms).forEach(function (e) {
  e.addEventListener(
    'submit',
    function (t) {
      if (e.checkValidity()) {
        t.preventDefault();
        var l = document.getElementById('foldername-input').value,
        ...
        if (
          ...
          if ('' !== l && editFlag) {
            var n;
            ((n = document.getElementById('folderid-input').value),
              (document
                .getElementById(n)
                .querySelector('.folder-name').innerHTML = l),
        ...
        ((document.getElementById('folderid-input').value = ''),
          (document.getElementById('foldername-input').value = ''));
      } else (t.preventDefault(), t.stopPropagation());
...
function editFileList() {
  ...
  Array.from(document.querySelectorAll('.edit-list')).forEach(function (t) {
    t.addEventListener('click', function (l) {
      ((e = t.getAttribute('data-edit-id')),
        (allFileList = allFileList.map(function (t) {
          return (
            t.id == e &&
              ((editFlag = !0),
              ...
              (document.getElementById('createFileModalLabel').innerHTML =
                'File Rename'),
              (document.getElementById('filename-input').value = t.fileName),
              (document.getElementById('fileid-input').value = t.id)),
            t
          );
        })));
    });
...
(Array.from(document.querySelectorAll('.createFile-modal')).forEach(
  function (e) {
        ...
        document
          ...
          .addEventListener('click', function () {
            var t,
              l =
                ((t = e),
                allFileList.filter(function (e) {
                  return e.id != t;
                }));
            (loadFileData((allFileList = l)),
...
function fileDetailShow() {
  ...
  (Array.from(document.querySelectorAll('.close-btn-overview')).forEach(
    function (t) {
      t.addEventListener('click', function () {
        e.classList.remove('file-detail-show');
      });
    },
  ),
    Array.from(document.querySelectorAll('#file-list tr')).forEach(
      function (t) {
        t.querySelector('.viewfile-list').addEventListener(
          'click',
          function () {
            (e.classList.add('file-detail-show'),
              (document.getElementById('file-overview').style.display =
                'block'),
              (document.getElementById('folder-overview').style.display =
                'none'));
            var l = t.querySelector('.filelist-id').value,
              i = t.querySelector('.filelist-icon i').className,
              n = t.querySelector('.filelist-name').innerHTML,
              o = t.querySelector('.filelist-size').innerHTML,
              r = t.querySelector('.filelist-create').innerHTML,
              d = t.querySelector('.filelist-type').innerHTML;
            ((document.querySelector('#file-overview .file-icon i').className =
              ...
              document
                ...
                .setAttribute('data-remove-id', l),
              t.querySelector('.favourite-btn').classList.contains('active')
                ? document
  ...
  (Array.from(document.querySelectorAll('.file-menu-btn')).forEach(
    function (e) {
      e.addEventListener('click', function () {
          ...
          document
            ...
            .addEventListener('click', function () {
              var t,
                l = '';
              ((t = e),
                (l = allFileList.filter(function (e) {
                  return e.id != t;
                })),
                loadFileData((allFileList = l)),
                document.getElementById('close-removefilemodal').click(),
...
(Array.prototype.slice.call(createFileForms).forEach(function (e) {
  e.addEventListener(
    'submit',
    function (t) {
      if (e.checkValidity()) {
        t.preventDefault();
        var l = document.getElementById('filename-input').value,
          i = Math.floor(100 * Math.random());
        if ('' === l || editFlag) {
          if ('' !== l && editFlag) {
            var n;
            ((n = document.getElementById('fileid-input').value),
