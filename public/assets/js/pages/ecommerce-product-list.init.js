...
(searchProductList.addEventListener('keyup', function () {
  ...
  var i = t(productListAllData, e),
    r = t(productListPublishedData, e);
  (productListAll.updateConfig({ data: i }).forceRender(),
    productListPublished.updateConfig({ data: r }).forceRender(),
    checkRemoveItem());
}),
  Array.from(document.querySelectorAll('.filter-list a')).forEach(function (e) {
    e.addEventListener('click', function () {
      var t = document.querySelector('.filter-list a.active');
      (t && t.classList.remove('active'), e.classList.add('active'));
      var i = e.querySelector('.listname').innerHTML,
        r = productListAllData.filter((e) => e.product.category === i),
        s = productListPublishedData.filter((e) => e.product.category === i);
      (productListAll.updateConfig({ data: r }).forceRender(),
        productListPublished.updateConfig({ data: s }).forceRender(),
        checkRemoveItem());
    });
  }));
...
Array.from(
  ...
  ((e.querySelector('.filter-badge').innerHTML = t),
    Array.from(e.querySelectorAll('.form-check .form-check-input')).forEach(
      function (i) {
        var r = i.value;
        (i.checked && filterChoicesInput.setValue([r]),
          i.addEventListener('click', function (s) {
            i.checked
              ? (t++,
                (e.querySelector('.filter-badge').innerHTML = t),
                (e.querySelector('.filter-badge').style.display =
                  t > 0 ? 'block' : 'none'),
                filterChoicesInput.setValue([r]))
              : filterChoicesInput.removeActiveItemsByValue(r);
          }),
...
searchBrandsOptions.addEventListener('keyup', function () {
  var e = searchBrandsOptions.value.toLowerCase(),
    t = document.querySelectorAll('#flush-collapseBrands .form-check');
  Array.from(t).forEach(function (t) {
    var i = t
      .getElementsByClassName('form-check-label')[0]
      .innerText.toLowerCase();
    t.style.display = i.includes(e) ? 'block' : 'none';
  });
});
var isSelected = 0;
        ...
        document
          ...
          .addEventListener('click', function () {
            (Array.from(document.querySelectorAll('.gridjs-table tr')).forEach(
              function (e) {
                var t = '';
                if (e.classList.contains('gridjs-tr-selected')) {
                  var i = e.querySelector('.form-check-input').value;
                  function r(e, t) {
                    return e.filter(function (e) {
                      return e.id != t;
                    });
                  }
                  t = r(productListAllData, i);
                  var s = r(productListPublishedData, i);
                  ((productListAllData = t),
                    (productListPublishedData = s),
                    e.remove());
                }
              },
            ),
      ...
      ((e = t.getAttribute('data-id')),
        document
          ...
          .addEventListener('click', function () {
            ...
            var r = i(productListAllData, e),
              s = i(productListPublishedData, e);
            ((productListAllData = r),
              (productListPublishedData = s),
              t.closest('.gridjs-tr').remove());
          }));
    });
  });
  var t = 0;
