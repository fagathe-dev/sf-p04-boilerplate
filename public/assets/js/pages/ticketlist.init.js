...
function SearchData() {
  var e = document.getElementById('idStatus').value,
    ...
    a = t.split(' to ')[0],
    i = t.split(' to ')[1];
  (ticketsList.filter(function (s) {
    matchData = new DOMParser().parseFromString(s.values().status, 'text/html');
    ...
    return (
      (n = 'all' == l || 'all' == e || l == e),
      (d =
        new Date(s.values().create_date.slice(0, 12)) >= new Date(a) &&
        new Date(s.values().create_date.slice(0, 12)) <= new Date(i)),
      n && d ? n && d : n && '' == t ? n : d && '' == t ? d : void 0
    );
  }),
    ticketsList.update());
}
      ...
      if (e.checkValidity())
        if (
          ...
          if (
            ...
            var a = ticketsList.get({ id: idField.value });
  ...
  (removeBtns &&
    Array.from(removeBtns).forEach(function (e) {
      e.addEventListener('click', function (e) {
        ...
        var t = ticketsList.get({ id: itemId });
        ...
        e.addEventListener('click', function (e) {
          ...
          Array.from(t).forEach(function (e) {
            ...
            if (t == itemId) {
              ...
              var a = val.body.firstElementChild.innerHTML;
...
function deleteMultiple() {
  ...
  for (i = 0; i < e.length; i++)
    if (1 == e[i].checked) {
      var t = e[i].parentNode.parentNode.parentNode
...
function ViewTickets(e) {
  ...
  s.innerHTML = i;
  var l = s.innerText.slice(4);
  (localStorage.setItem('ticket-list', JSON.stringify(a[0]._values)),
    localStorage.setItem('option', 'view-ticket'),
    localStorage.setItem('ticket_no', l),
