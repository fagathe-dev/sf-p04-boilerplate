...
function loadJobListData(e, t) {
  ...
  for (
    var o = (t - 1) * itemsPerPage;
    o < t * itemsPerPage && o < e.length;
    o++
  ) {
    if (e[o] && e[o].tags) {
      var a = e[o].tags,
...
function paginationEvents() {
  ...
  (prevButton.addEventListener('click', function () {
    currentPage > 1 &&
      (currentPage--, loadJobListData(allJobList, currentPage));
  }),
...
function findNextId() {
  if (0 === allJobList.length) return 0;
  var e = fetchIdFromObj(allJobList[allJobList.length - 1]),
...
function jobDetailShow() {
  Array.from(document.querySelectorAll('#job-list .joblist-card')).forEach(
    function (e) {
      e.querySelector('.viewjob-list').addEventListener('click', function () {
        var t = e.querySelector('.cover-img').src,
          n = e.querySelector('.companyLogo-img').src,
          o = e.querySelector('.job-title').innerHTML,
          a = e.querySelector('.company-name').innerHTML,
          i = e.querySelector('.job-description').innerHTML,
          r = e.querySelector('.job-type').innerHTML,
          l = e.querySelector('.job-location').innerHTML,
          c = e.querySelector('.job-postdate').innerHTML,
          s = e.querySelector('.job-experience').innerHTML;
        ((document.querySelector('#cover-img').src = t),
          (document.querySelector('#job-overview .view-companylogo').src = n),
          (document.querySelector('#job-overview .view-title').innerHTML = o),
          ...
          (document.querySelector('#job-overview .view-location').innerHTML =
            l),
          (document.querySelector('#job-overview .view-desc').innerHTML = i),
          (document.querySelector('#job-overview .view-type').innerHTML = r),
...
(document
  ...
  (function () {
    ...
    var e = document.querySelectorAll('.needs-validation'),
      t = new Date().toUTCString().slice(5, 16);
    Array.prototype.slice.call(e).forEach(function (e) {
      e.addEventListener(
        ...
        function (n) {
          if (e.checkValidity()) {
            n.preventDefault();
            var o = document.getElementById('jobtitle-field').value,
              a = document.getElementById('companyname-field').value,
              i = document.getElementById('companylogo-img').src,
              r = document.getElementById('modal-cover-img').src,
              l = document.getElementById('job_type-field').value,
              c = document.getElementById('experience-field').value,
              s = document.getElementById('location-field').value,
              d = document.getElementById('description-field').value,
              u = tagInputField.getValue(!0);
            if (
              '' !== o &&
              '' !== a &&
              '' !== l &&
              ...
              (allJobList.push(m), sortElementsById());
            }
            (loadJobListData(allJobList, currentPage),
              document.getElementById('close-jobListModal').click());
          } else (n.preventDefault(), n.stopPropagation());
...
(searchElementList.addEventListener('keyup', function () {
  ...
  var o = document.getElementById('page-num');
  o.innerHTML = '';
  for (var a = Math.ceil(n.length / itemsPerPage), i = 1; i < a + 1; i++)
    o.innerHTML +=
      "<div class='page-item'><a class='page-link clickPageNumber' href='javascript:void(0);'>" +
      i +
      '</a></div>';
