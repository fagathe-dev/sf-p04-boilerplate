  ...
  (Array.from(document.querySelectorAll('.message-list a')).forEach(
    function (t) {
      t.addEventListener('click', function (t) {
        (e.classList.add('email-detail-show'),
          Array.from(
    ...
    Array.from(document.querySelectorAll('.close-btn-email')).forEach(
      function (t) {
        t.addEventListener('click', function () {
          e.classList.remove('email-detail-show');
        });
      },
    ));
  var t = !1,
...
function checkBoxAll() {
  ...
  (Array.from(e).forEach(function (e) {
    e.addEventListener('click', function (e) {
      ...
      ((a.checked = l > 0),
        (a.indeterminate = l > 0 && l < t.length),
        e.target.closest('li').classList.contains('active'),
        (document.getElementById('email-topbar-actions').style.display =
          l > 0 ? 'block' : 'none'));
    });
  }),
    (document.getElementById('checkall').onclick = t));
}
(getJSON('mail-list.init.json', function (e, t) {
  ...
  Array.from(document.querySelectorAll('.mail-list a')).forEach(function (e) {
    e.addEventListener('click', function () {
      var t = document.querySelector('.mail-list a.active');
      if (
        (t && t.classList.remove('active'),
        e.classList.add('active'),
        e.querySelector('.mail-list-link').hasAttribute('data-type'))
      )
        var a = e.querySelector('.mail-list-link').innerHTML,
          l = allmaillist.filter((e) => e.labeltype === a);
      else {
        a = e.querySelector('.mail-list-link').innerHTML;
        if (((document.getElementById('mail-list').innerHTML = ''), 'All' != a))
          l = allmaillist.filter((e) => e.tabtype === a);
        else l = allmaillist;
        document.getElementById('mail-filter-navlist').style.display =
          'All' != a && 'Inbox' != a ? 'none' : 'block';
      }
      (loadMailData(l), favouriteBtn());
    });
  }),
  favouriteBtn(),
          ...
          (Array.from(document.querySelectorAll('.message-list li')).forEach(
            function (e) {
              ...
              if (e.classList.contains('active')) {
                var l = e.querySelector('.form-check-input').value;
                a =
                  ((t = l),
                  allmaillist.filter(function (e) {
                    return e.id != t;
                  }));
                ((allmaillist = a), e.remove());
              }
            },
          ),
        ...
        document
          ...
          .addEventListener('click', function () {
            var t,
              a = '';
            ((t = e),
              (a = allmaillist.filter(function (e) {
                return e.id != t;
              })),
              loadMailData((allmaillist = a)),
...
markAllReadBtn.addEventListener('click', function (e) {
  if (0 === document.querySelectorAll('.message-list li.unread').length) {
    ...
    function (e) {
      e.addEventListener('click', function () {
        ...
        document
          ...
          .setAttribute('data-remove-id', t);
        var a = e.querySelector('.subject-title').innerHTML;
        document.querySelector('.email-subject-title').innerHTML = a;
        var l = e.querySelector('.title-name').innerHTML;
        Array.from(document.querySelectorAll('.accordion-item.left')).forEach(
          function (t) {
            t.querySelector('.email-user-name').innerHTML = l;
            var a = e.querySelector('.mail-userimg').value;
            t.querySelector('img').setAttribute('src', a);
          },
        );
        var i = document.querySelector('.user-name-text').innerHTML,
          c = document
            .querySelector('.header-profile-user')
            .getAttribute('src');
        Array.from(document.querySelectorAll('.accordion-item.right')).forEach(
          function (e) {
            ((e.querySelector('.email-user-name-right').innerHTML = i),
              e.querySelector('img').setAttribute('src', c));
          },
        );
      });
    },
...
(document.querySelectorAll('.email-chat-list a').forEach(function (e) {
  if (e.classList.contains('active')) {
  ...
  e.addEventListener('click', function (t) {
    ((document.getElementById('emailchat-detailElem').style.display = 'block'),
      (mailChatDetailElm = !0));
    var a = document.querySelector('.email-chat-list a.active');
    (a && a.classList.remove('active'), this.classList.add('active'));
    scrollToBottom('users-chat');
    var l = e.querySelector('.chatlist-user-name').innerHTML,
      i = e.querySelector('.chatlist-user-image img').getAttribute('src');
    document.querySelector('.email-chat-detail .profile-username').innerHTML =
      l;
    var c = document.getElementById('users-conversation');
    Array.from(c.querySelectorAll('.left .chat-avatar')).forEach(function (e) {
      i
...
function resizeEvent() {
  if (document.documentElement.clientWidth < 767) {
    var e = document.querySelector('.email-chat-list a.active');
